...
class Bot:
    ...
    def find_blitziums_hostile(self, game_message: TeamGameState) -> Set[Tuple[int, int]]:
        ...
        for item in game_message.items:
            position = item.position
            if game_message.teamZoneGrid[position.x][position.y] != "" and game_message.teamZoneGrid[position.x][
                position.y] != game_message.currentTeamId and (
    ...
    def find_optimal_path_team_zone_next(self, start: Tuple[int, int], target_zones: Set[Tuple[int, int]],
                                    game_message: TeamGameState) -> List[Tuple[int, int]]:
        """Find the safest path to the nearest safe zone using A* algorithm"""
        if not target_zones:
            return []
        ...
        # Find nearest safe zone
        teamzone = min(target_zones, key=lambda sz: heuristic(start, sz))
        print(f"nearest_mineral:{teamzone}")
        return self.find_optimal_path(start, game_message, teamzone)
    ...
    def get_next_move(self, game_message: TeamGameState):
        ...
        for character in game_message.yourCharacters:
            ...
            if character.numberOfCarriedItems > 0:
                print(f"character holding:{character.carriedItems}")
            ...
            if character.numberOfCarriedItems > 0 and (
                ...
                if character.carriedItems[0].type.startswith("blitzium") and game_message.teamZoneGrid[current_pos[0]][
                    ...
                    if coordinates not in item_locations:
                ...
                if coordinates in item_locations and game_message.teamZoneGrid[current_pos[0]][
                    ...
                    if character.id not in self.current_path or len(self.current_path[character.id]) < 2:
                        if len(neutral_minerals) > 0 or character.numberOfCarriedItems > 0 and character.carriedItems[
                            0].type.startswith("blitzium"):
                            print("fetching blitzium")
                            if character.numberOfCarriedItems > 0 and character.carriedItems[0].type.startswith(
                                    "blitzium"):
                                player_zones = self.find_player_empty_squares(game_message)
                                ...
                                self.current_path[character.id] = self.find_optimal_path_mineral(current_pos,
                                                                                                 neutral_minerals,
                                                                                                 game_message)
                        elif len(red_minerals) > 0 or character.numberOfCarriedItems > 0 and character.carriedItems[
                            0].type.startswith("radiant"):
                            ...
                            if character.numberOfCarriedItems > 0 and character.carriedItems[0].type.startswith(
                                ...
                                self.current_path[character.id] = self.find_optimal_path_mineral(current_pos,
                                                                                                 red_minerals,
                                                                                                 game_message)
                        elif len(hostile_minerals) > 0 or character.numberOfCarriedItems > 0 and character.carriedItems[
                            0].type.startswith("blitzium"):
